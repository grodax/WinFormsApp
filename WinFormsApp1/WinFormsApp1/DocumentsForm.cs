...
namespace WinFormsApp1
{
    public partial class DocumentsForm : Form
    {
        ...
        private void LoadDocuments()
        {
            try
            {
                string sql = @"SELECT d.DocumentID, d.DocumentNumber, 
                          c.Name as CounterpartyName, d.DocumentDate, d.TotalAmount
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            string sql = @"INSERT INTO DocumentHeaders 
                      (DocumentNumber, CounterpartyID, DocumentDate, TotalAmount) 
                      VALUES (@Number, @CounterpartyID, @Date, 0)";
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        string sql = @"INSERT INTO DocumentSpecifications 
                                  ...
                                  VALUES (@DocID, @ProdID, @Qty, @ResQty, @Price, @Discount)";

                        SQLiteParameter[] parameters = {
                        new SQLiteParameter("@DocID", documentId),
                        new SQLiteParameter("@ProdID", productId),
                        new SQLiteParameter("@Qty", quantity),
                        new SQLiteParameter("@ResQty", quantity), // Резервируем всё количество
